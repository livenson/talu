...
|---|---|---|---|---|---|---|---|
| **Talu** | KubeVirt on Talos + Cilium + CephFS | **K8s API + Prometheus** (no proprietary API) | namespace + `HelmRelease` tenant, per-tenant Cilium policy | **no management portal** (orchestrator-agnostic; metrics dashboards via Perses, optional kubevirt-manager) | **yes** (Flux) | **lean** | OSS clone-and-adjust (MIT) |
